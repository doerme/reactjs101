...
详细的程式码读者可以参考范例资料夹，若读者跟著范例完成的话，可以在终端机上执行 `npm start`，并于浏览器 `http://localhost:8008`看到以下成果，当你点选连结时会切换对应元件并改变 `actived` 状态！
